...
export default class Mensajes extends Component {
    ...
    componentDidMount(){
        ...
        db.child(`convos/${this.state.convo}/messages`).on('value', snapConvos => {
            let msgArray = [];
    ...
    render() {
        return (
            <div>
                ...
                <Input.TextArea onChange={(e) => this.setState({msgText: e.target.value})} value={this.state.msgText} />
    ...
    handleSend = () => {
        let date = new Date();
        let genKey = uid(10);
        db.child(`convos/${this.state.convo}/messages`).push({
            key: genKey,
